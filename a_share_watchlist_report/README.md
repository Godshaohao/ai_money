...
- Adds a SQLite-backed strategy review loop with CLI access for limit-up, watchlist, and holding-risk review.
...
After `python run_report.py`, run the strategy review loop:

```bash
python -m src.cli.strategy run all
python -m src.cli.strategy list-runs
python -m src.cli.strategy inspect 002115
python -m src.cli.strategy export --path output/strategy_candidates.csv
```
...
The CLI writes local analysis records to `data/workbench.sqlite` and exports review candidates only. It does not output `BUY`, `SELL`, target prices, position sizes, broker actions, or automated orders.

## Local AI Agent CLI

Codex or another local AI agent can call the same application through deterministic JSON commands:

```bash
python -m src.cli.ai manifest --json
python -m src.cli.app status --json
python -m src.cli.report summary --json
python -m src.cli.report run --json
python -m src.cli.strategy run all --json
python -m src.cli.strategy inspect 002115 --json
python -m src.cli.analysis stock 603538 --json
python -m src.cli.strategy export --path output/strategy_candidates.csv --json
```
...
The `safety` block marks the output as analysis-only and lists forbidden outputs: `BUY`, `SELL`, `target_price`, `broker_order`, and `automated_trading`.

CLI calls are audited in the local SQLite table `cli_tool_runs`.

For richer single-stock review, prefer `analysis stock` over raw `strategy inspect`. It includes:

- identity
- data quality flags
- recent limit-up events
- sector echelon using the local `industry` field
- strategy candidates and evidence
- dragon-tiger rows when available
